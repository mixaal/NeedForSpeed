...
SubShader {
    ...
    Pass {
        ...
        inline float2 ToRadialCoords(float3 coords)
        {
            ...
            float latitude = acos(normalizedCoords.y*1.7);
            float longitude = 3*atan2(normalizedCoords.z, normalizedCoords.x);
            float2 sphereCoords = float2(longitude, latitude) * float2(0.5/UNITY_PI, 2.0/UNITY_PI);
...
CustomEditor "SkyboxCustomPanoramicShaderGUI"
Fallback Off
